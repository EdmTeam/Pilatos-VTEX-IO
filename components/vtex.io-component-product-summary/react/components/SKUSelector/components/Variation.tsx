...
const Variation: FC<Props> = ({
  ...
  let settings = {
    // slidesToShow: (sliderDisplayThreshold as number),
    // slidesToScroll: (sliderItemsPerPage as number),
    slidesToShow: 4,
    slidesToScroll: 1,
    dots: false,
    infinite: false,
  ...
  return (
    <div className={containerClasses}>
      <div className={`${styles.skuSelectorNameContainer} ma1`}>
        ...
        <div
          className={`${styles.skuSelectorOptionsList} ${displayOptions.length > 4 ? 'pilatos21-product-summary-0-x-contentSlide-more' : ''} w-100 inline-flex flex-wrap ml2 items-center`}
        >
          {mode === 'select' && !displayImage ? (
            ...
            />
          ) : shouldUseSlider && isMobile ? (
              // <SliderLayout {...sliderConfigurationProps}>
              //   {selectorItemsArray}
              // </SliderLayout>
              <div 
                className={`pilatos21-product-summary-0-x-contentSlide ${displayOptions.length > 4 ? 'pilatos21-product-summary-0-x-contentSlide-more' : ''}`}
              >
                <Slider {...settings}>
                  {selectorItemsArray}
                </Slider>
